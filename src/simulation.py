...
# UI elements
reveal_cards_button_rect = pygame.Rect(BUTTON_MARGIN, BUTTON_MARGIN, BUTTON_WIDTH, BUTTON_HEIGHT)
speed_button_rect = pygame.Rect(2 * BUTTON_MARGIN + BUTTON_WIDTH, BUTTON_MARGIN, BUTTON_WIDTH, BUTTON_HEIGHT)
start_button_rect = pygame.Rect(3 * BUTTON_MARGIN + 2 * BUTTON_WIDTH, BUTTON_MARGIN, BUTTON_WIDTH, BUTTON_HEIGHT)

def draw_button(screen: pygame.Surface, rect: pygame.Rect, text: str, is_active: bool = False, is_hover: bool = False):
...
def display_start_menu(screen: pygame.Surface):
    ...
    for i in range(5):
        ...
        title_rect = title_surface.get_rect(center=(WINDOW_WIDTH//2, WINDOW_HEIGHT//2 - 50 + i))
        screen.blit(title_surface, title_rect)
    
    # Draw subtitle
    subtitle_surface = font.render(subtitle, True, COLORS['text'])
    subtitle_rect = subtitle_surface.get_rect(center=(WINDOW_WIDTH//2, WINDOW_HEIGHT//2 + 20))
    screen.blit(subtitle_surface, subtitle_rect)
    
    # Draw start button with hover effect
    start_button = pygame.Rect(WINDOW_WIDTH//2 - BUTTON_WIDTH//2, 
                             WINDOW_HEIGHT//2 + 80, 
                             BUTTON_WIDTH, 
                             BUTTON_HEIGHT)
...
def display_reveal_cards_button(screen: pygame.Surface, reveal_cards_button_rect: pygame.Rect):
    ...
    draw_button(screen, reveal_cards_button_rect, 
                "Hide Cards" if reveal_player_cards else "Show Cards",
                reveal_player_cards, is_hover)
...
def display_ui_and_info(screen: pygame.Surface):
    ...
    pygame.draw.line(screen, COLORS['border'], (0, BOARD_TOP), (WINDOW_WIDTH, BOARD_TOP), 2)
    ...
    # Draw info panel
    info_rect = pygame.Rect(WINDOW_WIDTH - 400, BUTTON_MARGIN, 380, BOARD_TOP - 2 * BUTTON_MARGIN)
...
def display_info(screen: pygame.Surface, info_rect: pygame.Rect):
    ...
    for i, action in enumerate(last_actions):
        action_rect = pygame.Rect(info_rect.left + 15, 
                                title_rect.bottom + 15 + i * 35,
                                info_rect.width - 30, 30)
        if i % 2 == 0:
        ...
        text_rect = text_surface.get_rect(midleft=(action_rect.left + 10, action_rect.centery))
        screen.blit(text_surface, text_rect)
...
def display_speed_button(screen: pygame.Surface, speed_button_rect: pygame.Rect):
    ...
    draw_button(screen, speed_button_rect, 
                f"{moves_per_second} moves/s",
                False, is_hover)

def display_text(screen: pygame.Surface, text: str, x: int, y: int, font: pygame.font.Font, color=None):
    if color is None:
        color = COLORS['text']
...
def display_card(screen: pygame.Surface, card: Card, x: int, y: int, reveal_card=None, is_dead=False):
...
def display_player_info(screen: pygame.Surface, player: Player, x: int, y: int, is_next_player: bool):
    ...
    display_text(screen, player.name, info_rect.centerx, info_rect.centery - 12, small_font, name_color)
    display_text(screen, f"{player.coins} coins", info_rect.centerx, info_rect.centery + 12, small_font)

def display_player_zone(screen: pygame.Surface, player: Player, x: int, y: int, player_zone_width: int=200, player_zone_height: int=100, player_index: int=0, is_next_player: bool=False):
    # Draw player zone background with a subtle gradient
    player_zone = pygame.Rect(x, y, player_zone_width, player_zone_height)
    
    # Create gradient effect
    gradient_surface = pygame.Surface((player_zone_width, player_zone_height), pygame.SRCALPHA)
    color1 = COLORS['board']
    color2 = tuple(max(0, c - 20) for c in color1)  # Slightly darker
    
    for i in range(player_zone_height):
        progress = i / player_zone_height
        current_color = tuple(int(c1 * (1 - progress) + c2 * progress) 
                            for c1, c2 in zip(color1, color2))
        pygame.draw.line(gradient_surface, current_color, 
                        (0, i), (player_zone_width, i))
    ...
    if player_index % 2 == 0:  # Top or bottom player
        ...
        for i, card in enumerate(player.hand):
            ...
            display_card(screen, card, card_x, card_y, 
                        reveal_card=reveal_player_cards,
...
def display_board(board: Board):
    ...
    display_deck(screen, board.deck, 
    ...
    for i, player in enumerate(board.players):
        ...
        if i == 0:  # Top player
            ...
            x = board_zone_rect.x
            y = board_zone_rect.y + board_zone_rect.height / 2 - player_zone_height / 2
        display_player_zone(screen, player, x, y, player_zone_width, player_zone_height, player_index=i, is_next_player=board.next_player == player)
